...
const fetchNotionData = async <T extends any>({
  ...
  const res = await axios.post<T>(`${NOTION_API}/${resource}`,
  JSON.stringify(body)
  , { 
    headers: {
...
export const fetchNotionUsers = async (
  ...
  const users = await fetchNotionData<{ readonly results: readonly NotionUserType[] }>({
...
export const fetchNotionSearch = async (
  ...
  return fetchNotionData<{ readonly results: NotionSearchResultsType }>({
    resource: "search",
    body: {
